...
export default function Municipales() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/dashboard/municipales/stats?days=${days}`, {
  ...
  const actividadOptions = useMemo(() => ({
    ...
    xaxis: {
      categories: data?.charts.actividad.labels ?? [],
      labels: { rotate: -45, hideOverlappingLabels: true, style: { colors: DARK.muted, fontSize: "10px" } },
  ...
  const actividadSeries = useMemo(() => [
    { name: "Movimientos", data: data?.charts.actividad.values ?? [] }
  ], [data]);
  ...
  const barOpts = (labels: string[], horizontal = true) => ({
    ...
    xaxis: {
      categories: labels,
      labels: { style: { colors: DARK.muted, fontSize: "11px" } },
      axisBorder: { show: false }
    },
    yaxis: { labels: { style: { colors: DARK.muted, fontSize: "11px" } } },
    colors: [DARK.accent],
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ marginBottom: 16, paddingBottom: 12, borderBottom: "2px solid #dee2e6" }}
      >
        <div style={{ fontSize: 20, fontWeight: 700 }}>
          Estadísticas Operativas · Municipales
        </div>
        <div style={{ fontSize: 12, color: "#6c757d", marginTop: 4 }}>
          Período: últimos {days} días · Impreso el{" "}
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 12, color: DARK.muted, margin: "2px 0 0" }}>
            Actividad y rendimiento de volquetes municipales
          </p>
        </div>
          ...
          <>
            ...
            >
              ...
              >
                ...
                >
                  Actividad operativa · movimientos por día
                </div>
            ...
            >
              ...
              >
                ...
                >
                  Top volquetes · más movimientos en el período
                </div>
            ...
            <div
              className="print-grid-2col"
              style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}
            >
              <div
                className="print-surface print-no-break"
                style={{
                  background: DARK.surface,
                  border: `1px solid ${DARK.border}`,
                  borderRadius: 10,
                    ...
                    <thead>
                      <tr className="print-muted" style={{ color: DARK.muted }}>
                        {["Fecha", "Volquete", "Tipo", "Ubicación nueva", "Nota"].map(h => (
                          <th key={h} style={{ textAlign: "left", padding: "6px 8px 10px", fontWeight: 600, fontSize: 11 }}>
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {data.tables.ultimosMovimientos.map((m) => (
                        <tr key={m.id} style={{ borderTop: `1px solid ${DARK.border}` }}>
                          <td className="print-muted" style={{ padding: "8px", whiteSpace: "nowrap", color: DARK.muted }}>
                            {formatDate(m.fecha)}
                          </td>
                          ...
                          <td className="print-muted" style={{ padding: "8px", color: DARK.muted }}>
                            {m.ubicacion_nueva ?? "-"}
                          </td>
                          <td className="print-muted" style={{ padding: "8px", color: DARK.muted }}>
                            {m.nota ?? "-"}
                      ...
                      {data.tables.ultimosMovimientos.length === 0 && (
                        <tr>
                          <td colSpan={5} style={{ padding: "20px 8px", color: DARK.muted, textAlign: "center" }}>
                            Sin movimientos
                          </td>
                        </tr>
                      )}
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    <tbody>
                          ...
                          <td style={{ padding: "8px" }}>
                            <span
                              ...
                              style={{
                                fontFamily: "'DM Mono', monospace",
                                fontWeight: 700,
                                color: v.dias_colocado && v.dias_colocado > 7 ? DARK.yellow : DARK.text
                              }}
                            >
                              {v.dias_colocado ?? "-"}d
                            </span>
                      ...
                      {data.tables.municipalesColocados.length === 0 && (
                        <tr>
                          <td colSpan={4} style={{ padding: "20px 8px", color: DARK.muted, textAlign: "center" }}>
                            Ningún municipal colocado
                          </td>
                        </tr>
                      )}
                    </tbody>
